...
You safely can reuse previous instances, since every method returns a new
instance and does not modify the previous one.
...
You can provide an assertion function to `json` resolver in order to perform a
runtime validation. Otherwise, the result is going to be `unknown`.
