...
function setMessage(config: XShieldError) {
  const message = StatusCodes.has(config.status) && StatusCodes.get(config.status);
...
export function assertXShieldError(error: unknown): asserts error is XShieldError {
