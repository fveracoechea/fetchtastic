...
The `DataGrabError` class extends the built-in `Error` class and represents an error that occurs during an HTTP request made with `DataGrab`. It encapsulates information about the error, including the request URL, HTTP method, response details, and error message.
...
Creates a new instance of the `DataGrabError` class.
...
The `DataGrabError` class does not expose any public methods.
