[fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) does not
reject on http error status, so manually checking and handling every request
error code can be very tedious:
...
The original config is passed along the error and can be used in order to
perform an additional request, and modify the initial response:
...
In the above example, you can notice that the execution flow is preserved as
expected, `.then` will be called with the result of the original request or the
replayed one.
